...
def segment_floor_points(
    ...
    *,
    ...
    log.info(
        "Floor plane: normal=[%.4f, %.4f, %.4f], d=%.4f, scene_scale=%.3f, RANSAC inliers=%d",
        *normal,
        d,
        scene_scale,
        len(inliers),
    ...
    log.info(
        "Floor extension: tolerance=%.4f (scale=%.4f,min=%.4f), points on plane=%d / %d, distance quantiles=[p50=%.4f,p75=%.4f,p90=%.4f,p95=%.4f]",
        tolerance,
        extension_tolerance_scale,
        extension_tolerance_min,
        int(plane_mask.sum()),
        len(points),
        q50,
        q75,
